...
class MARSDL(VideoPersonDataset):
    """MARS.
    ...
    def __init__(self, root='', dense=False, sampling_step=32, new_eval=True, **kwargs):
        # self.root = osp.abspath(osp.expanduser(root))
        self.root = root
        self.dataset_dir = osp.join(self.root, 'MARS_DL')
        ...
        if self.new_eval:
            logger.info('Using new eval benchmark......')
            self.track_test_info_path = osp.join(self.dataset_dir, 'info/clean_tracks_test_info.mat')
        else:
            self.track_test_info_path = osp.join(self.dataset_dir, 'info/tracks_test_info.mat')
        ...
        train = self.process_data(train_names, track_train, home_dir='bbox_train', dense=dense, sampling_step=sampling_step)

        query = self.process_data(test_names, track_query, home_dir='bbox_test', dense=False, new_eval=new_eval)
        gallery = self.process_data(test_names, track_gallery, home_dir='bbox_test', dense=False, new_eval=new_eval)

        super(MARSDL, self).__init__(train, query, gallery)
    ...
    def process_data(self, names, meta_data, home_dir=None, min_seq_len=0, new_eval=False, dense=False, sampling_step=32):
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            if new_eval:
                start_index, end_index, pid, camid, new_pid, new_ambi = data
            ...
            frame_ids = [int(img_path.split('.')[0][-3:]) for img_path in img_paths]
            track_ids = [int(img_path.split('.')[0][-8:-4]) for img_path in img_paths][0]
            ...
            if dense:
                ...
                if num_sampling == 0:
                    if len(img_paths) >= min_seq_len:
                        ...
                        tracklets.append((img_paths, pid, camid, track_ids, frame_ids, ambi))
                else:
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            tracklets.append(
                                (img_paths[idx * sampling_step:], pid, camid, track_ids, frame_ids[idx * sampling_step:], ambi))
                        else:
                            tracklets.append((img_paths[idx * sampling_step: (idx + 1) * sampling_step], pid, camid,
                                              track_ids,
                                              frame_ids[idx * sampling_step: (idx + 1) * sampling_step], ambi))
