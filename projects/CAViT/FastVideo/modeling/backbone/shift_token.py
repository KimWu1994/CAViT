...
class VisionTransformer(nn.Module):
	...
	def forward(self, x, camera_id=None, frame_id=None):
		...
		# b, c = x.size()
		# x = x.view(b//self.n_seg, self.n_seg, -1)
		return x
...
def build_shift_token_vit(cfg):
    vit_cfg = CONFIGS['ViT-B_16']
    vit_cfg.n_seg = 8
    vit_cfg.fold_div = 4


    model = VisionTransformer(config=vit_cfg, 
                            img_size=224, 
                            zero_head=True, 
                            num_classes=21843)

    model.load_from(np.load('/disk1/jinlin/imagenet21k_ViT-B_16.npz'))   
    del model.head
