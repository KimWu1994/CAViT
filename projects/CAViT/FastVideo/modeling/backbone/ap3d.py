...
from FastVideo.modeling.layers import AP3D
from FastVideo.modeling.layers  import  NonLocal
...
__all__ = ['AP3DResNet50', 'AP3DNLResNet50', 'NLResNet50', 'C2DResNet50', 
           'C2DResNet18', 'NLResNet18',
          ]


class BasicBlock3D(nn.Module):
    def __init__(self, basicblock2d, block, inflate_time=False):
        ...
        if inflate_time == True:
            self.conv2 = block(basicblock2d.conv2)
        else:
...
class ResNet503D(nn.Module):
    def __init__(self, block, c3d_idx, nl_idx, **kwargs):
        super(ResNet503D, self).__init__()

        self.block = block
        self.temperature = 4
        self.contrastive_att = True
...
class ResNet3D(nn.Module):
    def __init__(self, config, num_layer, block, c3d_idx, nl_idx, **kwargs):
        super().__init__()
        ...
        self.bn1 = inflate.inflate_batch_norm(resnet2d.bn1)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = inflate.MaxPool2dFor3dInput(resnet2d.maxpool.kernel_size)
    ...
    def _inflate_reslayer(self, reslayer2d, c3d_idx, nonlocal_idx=[], nonlocal_channels=0):
        ...
        for i,layer2d in enumerate(reslayer2d):
            if i not in c3d_idx:
                layer3d = BasicBlock3D(layer2d, AP3D.C2D, inflate_time=False)
            else:
                layer3d = BasicBlock3D(layer2d, self.block, inflate_time=True, \
                                       temperature=self.temperature, contrastive_att=self.contrastive_att)
            reslayers3d.append(layer3d)
...
def build_c2d_backbone(cfg):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[],[],[]]
    return ResNet503D(AP3D.APP3DC, c3d_idx, nl_idx)


@BACKBONE_REGISTRY.register()
def build_ap3d_backbone(cfg):
    c3d_idx = [[],[0, 2],[0, 2, 4],[]]
    nl_idx = [[],[],[],[]]
    return ResNet503D(AP3D.APP3DC, c3d_idx, nl_idx)

@BACKBONE_REGISTRY.register()
def build_ap3d_nl_backbone(config):
    c3d_idx = [[],[0, 2],[0, 2, 4],[]]
    nl_idx = [[],[1, 3],[1, 3, 5],[]]
...
@BACKBONE_REGISTRY.register()
def build_r50_nl_backbone(config, **kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[1, 3],[1, 3, 5],[]]

    return ResNet503D(AP3D.APP3DC, c3d_idx, nl_idx, **kwargs)


def C2DResNet18(config, **kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[],[],[]]

    return ResNet3D(config, 18, AP3D.APP3DC, c3d_idx, nl_idx, **kwargs)


def NLResNet18(config, **kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[1],[1],[]]
