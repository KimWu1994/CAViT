...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, padding_size=0, in_chans=3, embed_dim=768,
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, padding_size=padding_size, stride_size=stride_size, in_chans=in_chans,
                embed_dim=embed_dim)
    ...
    def forward(self, x, camera_id=None, frame_id=None):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks  [batch, 1, 768]
        x = torch.cat((cls_tokens, x), dim=1)  # [b*t, p, c]

        if self.cam_num > 0:
            x = x + self.pos_embed + self.sie_xishu * self.sie_embed[camera_id[:, 0]]
        else:
            x = x + self.pos_embed

        x = self.pos_drop(x)  # [batch, 129, 768]

        for blk in self.blocks[:self.inference_depth]:
            x = blk(x)        # [batch, 129, 768]
...
def build_myvit_backbone(cfg):
    ...
    embed_dim       = cfg.MODEL.BACKBONE.FEAT_DIM
    # fmt: on
    ...
    model = VisionTransformer(img_size=input_size, sie_xishu=sie_xishu, stride_size=stride_size, padding_size=padding_size, depth=num_depth, embed_dim=embed_dim,
                              ...
                              inference_depth=inference_depth, layer_num=layer_num, norm_out=norm_out)
